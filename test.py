...
def face_embedding(det_model, img, ret_center=False):
    ...
    for face in det_res:
        ...
        print(box_c_x,img_center_x,box_c_x - img_center_x)
    ...
    if not ret_center:
        ...
        abs_dist = [abs(i) for i in center_d_list]
        print(abs_dist)
        center_sort = np.argsort(abs_dist)
...
img1 = cv2.imread("两个人识别了中间的.png")  # 读取图片
...
for face_i,face in enumerate(face_list):
    ...
    bbox = face['bbox']
    cv2.rectangle(output_img, (int(bbox[0]), int(bbox[1])), (int(bbox[2]), int(bbox[3])), color=(0, 255, 0), thickness=5)
